...
# --- GLOBAL CSS STYLING (Applies everywhere) ---
st.markdown("""
    <style>
...
if not st.session_state.run_analysis:
    ...
    # --- DYNAMIC CSS: PURE WHITE BACKGROUND ---
    st.markdown("""
        <style>
        .stApp {
        ...
        /* Ensure tab text, headers, and standard text turn dark for readability */
    ...
    # Fixed HTML font colors for a white background
    st.markdown("<h1 style='text-align: center; color: #000000; margin-top: 20px;'>Monte Carlo Option Pricing Model</h1>", unsafe_allow_html=True)
    st.markdown("<h4 style='text-align: center; color: #4f4f4f;'>Group 16 — Financial Engineering Dashboard</h4><br>", unsafe_allow_html=True)

    # --- EXPANDED NIFTY 50 TICKER MAPPING ---
    TICKER_MAPPING = {
    ...
    with col_center:
        # Changed background to a soft gray with dark borders and dark text to stand out against the white background
        st.markdown("""
        <div style="background-color: #f8f9fa; padding: 30px; border-radius: 8px; border: 1px solid #dee2e6;">
            <h3 style='color: #000000; text-align: center; margin-bottom: 20px;'>Model Configuration Parameters</h3>
        """, unsafe_allow_html=True)

        selected_company = st.selectbox(
            "Search and Select Company Name", 
            options=list(TICKER_MAPPING.keys()),
            index=37 # Defaults to Reliance
        )

        if selected_company == "Other (Manual Entry)":
            st.session_state.ticker_input = st.text_input("Enter Custom Yahoo Finance Ticker", value="AAPL")
        else:
            st.session_state.ticker_input = TICKER_MAPPING[selected_company]
            st.info(f"Assigned Ticker Symbol: **{st.session_state.ticker_input}**")
...
# ==========================================
    ...
    st.markdown("""
        ...
        /* Theory container styled for white background */
        .theory-container {
    ...
    with st.spinner("Processing computational models and executing Monte Carlo iterations..."):
        ...
        # EXACT MATCH LOGIC: Check for S0 in strikes, inject if missing
        S0_rounded = round(S0, 2)
        strikes = np.round(np.arange(0.80, 1.3, 0.05) * S0, 2)
        
        if S0_rounded not in strikes:
            strikes = np.append(strikes, S0_rounded)
    ...
    with tab_theory:
        ...
        **Variable Definitions:**
        ...
        st.subheader("3.6. Confidence Intervals (CI)")
        st.write(r"""
        A Confidence Interval provides a range of values within which we expect the true theoretical option price to fall, given the mathematical uncertainty of the simulation. Because the Monte Carlo method relies on random sampling, every run will yield a slightly different output. The CI quantifies this randomness. 

        The mathematical formula for defining the upper and lower bounds of a confidence interval is:
        
        $$ \text{CI} = \hat{V}_0 \pm z \cdot \text{SE} $$
        
        Where $z$ is the critical value extracted from the standard normal distribution corresponding to the desired confidence level.

        **Why is 95% the Standard Choice?**
        In statistical analysis and financial engineering, a 95% confidence level (where $z \approx 1.96$) is the universally accepted standard. It strikes an optimal balance between precision and reliability. A 95% CI means that if we were to execute the same Monte Carlo simulation 100 times, we would expect the true theoretical option premium to fall within our calculated bounds exactly 95 times. 
        
        * Requiring a stricter confidence level (like 99%) would force the interval to be too wide, making the price estimate practically useless for trading decisions. 
        * Conversely, a lower level (like 90%) creates a narrower band but significantly increases the risk that the true option value falls entirely outside of our estimated range.
        """)
        ...
        * **$z$:** The critical value for the confidence interval.
        """)
    ...
    with tab_table:
        ...
        st.markdown("""
        ---
        **Analytical Summary:** These charts and the accompanying matrix compare the option premiums calculated via the analytical Black-Scholes formula against the simulated Monte Carlo results across a range of strike prices. As expected, call values decline monotonically with increasing strike prices, while put values increase.
        """)
    ...
    with tab_charts:
        ...
        **Analytical Summary:**
        ...
        ax3.axhline(bs_ref_call, color="red", ls="--", lw=1.5, label=f"Black Sholes Target = Rs{bs_ref_call:.2f}")
        ...
        **Analytical Summary:**
        ...
        ax4.axhline(bs_ref_put, color="red", ls="--", lw=1.5, label=f"Black Sholes Target = Rs{bs_ref_put:.2f}")
